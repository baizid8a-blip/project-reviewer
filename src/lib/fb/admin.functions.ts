...
export const listAvailableAdAccounts = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    if (!token && existingByFb.size === 0) return [] as any[];
...
export const listAvailableAdSets = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const token = s?.fb_system_user_token;
    if (!token) {
      return { adsets: [], accounts: [], totalAccounts: 0, truncatedAccounts: 0, perAccountErrors: [], liveError: "No Facebook token configured. Open Meta connection settings first." };
    }
