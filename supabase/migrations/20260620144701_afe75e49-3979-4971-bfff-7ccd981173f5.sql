...
CREATE TABLE public.app_settings (
  ...
  fb_system_user_token TEXT, fb_app_id TEXT, fb_business_id TEXT, fb_verify_token TEXT, fb_app_secret TEXT,
  sync_interval_minutes INT NOT NULL DEFAULT 5, auto_sync_enabled BOOLEAN NOT NULL DEFAULT true,
  ...
  brand_logo_url TEXT, brand_primary_color TEXT DEFAULT '#1F2240', brand_secondary_color TEXT DEFAULT '#8B5CF6',
  pref_timezone TEXT DEFAULT 'Asia/Dhaka', pref_currency TEXT DEFAULT 'USD', pref_language TEXT DEFAULT 'en',
...
BEGIN
  LOOP
    result := '';
    FOR i IN 1..8 LOOP result := result || substr(chars, 1 + floor(random() * length(chars))::integer, 1); END LOOP;
...
CREATE TABLE public.ad_accounts (
  ...
  account_name TEXT, currency TEXT, timezone_name TEXT, account_status INT, business_name TEXT,
  total_spend NUMERIC(14,2) DEFAULT 0, total_reach BIGINT DEFAULT 0, total_impressions BIGINT DEFAULT 0,
  total_clicks BIGINT DEFAULT 0, total_results BIGINT DEFAULT 0, active_campaigns INT DEFAULT 0,
...
CREATE TABLE public.ad_sets (
  ...
  name TEXT NOT NULL, status public.entity_status, effective_status public.entity_status,
...
CREATE TABLE public.insights_snapshots (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ad_account_id UUID NOT NULL REFERENCES public.ad_accounts ON DELETE CASCADE,
  level public.insight_level NOT NULL, entity_id TEXT NOT NULL,
...
CREATE TABLE public.alerts (
  ...
  type TEXT NOT NULL, severity public.alert_severity NOT NULL DEFAULT 'info',
  title TEXT NOT NULL, message TEXT, is_read BOOLEAN NOT NULL DEFAULT false,
...
CREATE TABLE public.meta_webhook_events (
  ...
  payload JSONB NOT NULL, signature_valid BOOLEAN NOT NULL DEFAULT false,
...
CREATE POLICY "client_ad_sets_select_auth" ON public.client_ad_sets FOR SELECT TO authenticated USING (true);
CREATE POLICY "client_ad_sets_admin_modify" ON public.client_ad_sets FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::public.app_role)) WITH CHECK (public.has_role(auth.uid(), 'admin'::public.app_role));
CREATE TRIGGER client_ad_sets_updated_at BEFORE UPDATE ON public.client_ad_sets FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
